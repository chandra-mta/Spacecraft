...
sub bit {
  ...
  ###print "I AM HERE OFF: $bit, $n, $exp, $i\n";
  if ($bit + $len > 32) {
    ###print "I AM HERE BIT BUF: $buf\n";
    $n = $buf[$off >> 5] >> $bit;
    ###print "I AM HERE BIT N: $n\n";
    exit 1;
    $n &= 0x7fffffff >> ($bit - 1) if $bit;
    ###print "I AM HERE BIT N2: $n\n";
    $exp = 32 - $bit;
    $off += $exp;
    $len -= $exp;
    $bit = 0;
  ...
  $i = $buf[$off >> 5] >> $bit;
  ###print "I AM HERE BIT I:  $i , $off, $bit\n";
  $i &= 0x7fffffff >> (31 - $len) if $len < 32;
  ###print "I AM HERE BIT I2:  $i , $off, $bit\n";
  $n += $i << $exp;
  return $n;
}
